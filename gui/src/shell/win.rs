...
/// Capture the window's client area to `path` as a PNG and answer `reply`.
///
/// **Why `PrintWindow` and not WebView2's `CapturePreview`.** `CapturePreview`
/// is the "official" WebView2 answer, but it is ASYNCHRONOUS, and this module's
/// header records what that costs on this thread: an async WebView2 call
/// awaited from the UI thread runs a nested message loop and can deadlock (the
/// P4 finding that made `eval_js` fire-and-forget). `PrintWindow` with
/// `PW_RENDERFULLCONTENT` is synchronous, needs no completion handler, and
/// exists precisely to capture windows whose content is composed off the
/// legacy GDI path — which is what a WebView2 child is. So the UI thread does
/// the whole capture inline and answers immediately; nothing blocks but the
/// listener thread, which is a plain worker with nothing else to do.
///
...
fn capture_png(h: HWND, path: &str) -> std::result::Result<(), String> {
    unsafe {
        let mut rect = RECT::default();
        GetClientRect(h, &mut rect).map_err(|e| format!("GetClientRect: {e}"))?;
        let (w, ht) = (rect.right - rect.left, rect.bottom - rect.top);
        if w <= 0 || ht <= 0 {
            return Err("client area is empty".into());
        }
...
fn write_png(path: &str, w: u32, h: u32, rgba: &[u8]) -> std::result::Result<(), String> {
