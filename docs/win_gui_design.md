...
| WG5 | Workspace + Browser | L | WG4, image_store | **built, less 5c** (`just gate-wg5b`) — the two flagship views: Workspace with syntax colouring + ghost line + Ctrl-D/Ctrl-P; the four-pane Browser over the shared path-scheme model, Accept persisting to the image byte-identically to the web path (the CG8 gate, re-run) |
